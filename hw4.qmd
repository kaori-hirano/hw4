...
kmeans_out <- kmeans(d[train,] %>% select(-one_of('rank', 'ethnicity', 'gender','language', 'cls_level', 'cls_profs', 'cls_credits', 'pic_outfit', 'pic_color')), centers = 4, nstart = 50)
...
The ideal lambda is 0.026 The MSE for the test set is .259. The variables selected by lasso are tenured, ethnicity (not minority), gender (male), language (non english), class credit (one credit), outfit (not formal) and pic color (color). 
